-- ============================================================
-- Server-side messaging token enforcement
-- - Token balance helpers + deduct RPC
-- - start_conversation_with_message gates + deducts on new chats
-- - Tokens RLS: read-only for users (grants via SECURITY DEFINER RPCs)
-- ============================================================

-- 0. Legacy `token` column default (production rows require a unique value)
DO $$
BEGIN
  IF EXISTS (
...
-- 1. Token balance helper (non-expired rows only)
...
-- 2. Active subscription = unlimited messaging (monthly plans, not pay-per-use packs)
...
-- 3. Internal FIFO token deduction (callable from other SECURITY DEFINER functions)
...
-- 4. Public deduct RPC (authenticated users)
...
BEGIN
  ...
  -- p_token_type reserved for future token kinds; all messaging uses remaining_activations today
...
-- 5. Token balance RPC (matches production schema)
...
-- 6. Welcome bonus (replaces client-side INSERT blocked by RLS)
...
-- 7. Referral bonus for referrer (called by the newly signed-up user)
...
-- 8. start_conversation_with_message — enforce quota + deduct on new conversations
...
-- 9. Tokens RLS — read-only for end users
