-- ============================================================
-- client_type column on client_profiles + backfill + RPC update
-- ============================================================
...
AS $$
  SELECT
    ...
    p.avatar_url,
    COALESCE(to_jsonb(p.interests), cp.interests) AS interests,
    to_jsonb(p.lifestyle_tags) AS lifestyle_tags,
    p.smoking,
    COALESCE(p.work_schedule, cp.work_schedule) AS work_schedule,
    p.nationality,
    to_jsonb(p.languages_spoken) AS languages_spoken,
    COALESCE(p.neighborhood, cp.neighborhood) AS neighborhood,
    COALESCE(p.bio, cp.bio) AS bio,
    p.onboarding_completed,
    COALESCE(cp.profile_images, to_jsonb(p.profile_images)) AS profile_images,
