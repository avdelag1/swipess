      ...
      if (jsonMatch) {
        try {
          ...
          if (action?.type === "search_local_expert_knowledge" && action.params?.query) {
            ...
            const { data, error } = await supabase.from('matches').insert({ user_id: user.id, listing_id: action.params.listing_id, owner_id: action.params.owner_id || 'unknown' }).select().single();
            cleanMessages.push({ role: "assistant", content });
