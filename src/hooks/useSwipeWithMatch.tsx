  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      if (isLike && variables.targetType === 'profile') {
        ...
        // Client liking a listing/event: refresh both matches and the saved-likes list
        // so the item appears in /client/liked-properties immediately.
        Promise.all([
          queryClient.invalidateQueries({ queryKey: ['matches'] }),
          queryClient.invalidateQueries({ queryKey: ['liked-properties'] }),
          queryClient.invalidateQueries({ queryKey: ['client-stats'] }),
        ]).catch(err => logger.error('[useSwipeWithMatch] Like invalidation failed:', err));
      } else if (isDislike) {
