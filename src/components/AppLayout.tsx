...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  return (
    <div className={cn(
      ...
      {showAppChrome && (
        <Suspense fallback={null}>
          <SwipessHud side="top" className="fixed top-0 left-0 right-0 z-[40]" scrollTargetSelector=".dashboard-scroll-target" revealMode={useRevealMode}>
            <TopBar
              userRole={userRole}
      ...
      {showAppChrome && (
        <Suspense fallback={null}>
          <SwipessHud side="bottom" pointerEvents="auto" className="fixed bottom-0 left-0 right-0 z-[40]" scrollTargetSelector=".dashboard-scroll-target" revealMode={useRevealMode}>
            <BottomNavigation
