    ...
    (async () => {
      ...
      try {
        ...
        canvas.addEventListener('webglcontextlost', (ev) => {
          ...
          pageMapFallbackDone = true;
          if (import.meta.env.DEV) console.warn('[PassportMap] WebGL context lost — one legacy re-init only');
        ...
        map.on('error', (e: { error?: { status?: number; message?: string } }) => {
          ...
          if (/UBO|uniform block|WebGL|context lost|exceeds device limit/i.test(message)) {
            if (!profile.useLegacyGl && !pageMapFallbackDone) {
              pageMapFallbackDone = true;
              if (import.meta.env.DEV) console.warn('[PassportMap] WebGL error, one legacy fallback:', message);
              destroyMapInstance(map);
              initStartedRef.current = false;
              setMapReady(false);
              forceLegacyMapProfile();
    ...
    if (raster && pageRasterMode) {
      ...
      for (const item of items) {
        ...
        if (item.type === 'cluster') {
          ...
          // Start visible — Safari rAF opacity fade was leaving pins stuck at 0
          el.style.opacity = '1';
        ...
        if (pin.kind === 'listing') {
          ...
          if (existing && existing.pinType === 'listing') {
            ...
            existing.el.style.opacity = isSelected || !sel ? '1' : existing.el.style.opacity;
            continue;
          }
          if (existing) {
            existing.cleanup();
    ...
    for (const item of items) {
      ...
      if (pin.kind === 'listing') {
        ...
        if (existing && existing.pinType === 'listing') {
          ...
          if (!sel || isSelected) existing.el.style.opacity = '1';
          continue;
        }
        if (existing) {
          existing.cleanup();
