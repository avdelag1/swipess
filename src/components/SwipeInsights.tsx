...
export function SwipeInsights({ userRole, isOpen, onClose }: SwipeInsightsProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-background border border-border rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="sticky top-0 bg-background/95 backdrop-blur-xl border-b border-border p-6 rounded-t-3xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-foreground">Swipe Analytics</h2>
                <p className="text-sm text-muted-foreground">Your activity insights</p>
              </div>
            </div>
            <Button variant="ghost" onClick={onClose} className="text-muted-foreground hover:text-foreground h-11 w-11 rounded-full hover:bg-muted">
              <X className="w-6 h-6" />
            </Button>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card className="bg-gradient-to-br from-blue-500/15 to-blue-600/10 backdrop-blur-sm border-blue-500/30 hover:border-blue-400/50 transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/20">
              <CardContent className="p-5 text-center">
                ...
                <div className="text-3xl font-bold text-foreground mb-1">{metrics?.totalSwipes || 0}</div>
                <div className="text-muted-foreground text-sm font-medium">Total Swipes</div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-gradient-to-br from-red-500/15 to-orange-600/10 backdrop-blur-sm border-red-500/30 hover:border-red-400/50 transition-all duration-300 hover:shadow-lg hover:shadow-red-500/20">
              <CardContent className="p-5 text-center">
                ...
                <div className="text-3xl font-bold text-foreground mb-1">{metrics?.likesGiven || 0}</div>
                <div className="text-muted-foreground text-sm font-medium">Likes Given</div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-gradient-to-br from-purple-500/15 to-pink-600/10 backdrop-blur-sm border-purple-500/30 hover:border-purple-400/50 transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/20">
              <CardContent className="p-5 text-center">
                ...
                <div className="text-3xl font-bold text-foreground mb-1">{metrics?.averageSessionTime || 0}m</div>
                <div className="text-muted-foreground text-sm font-medium">Avg Session</div>
              </CardContent>
            </Card>
          </div>
            ...
            <CardHeader>
              <CardTitle className="text-foreground flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground font-normal">Track your matching success</div>
                </div>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground font-medium">Current Rate</span>
                ...
                <div className="relative">
                  <Progress value={metrics?.matchRate || 0} className="h-3 bg-muted" />
            ...
            <CardHeader>
              <CardTitle className="text-foreground flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground font-normal">Your weekly performance</div>
                </div>
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="bg-card/50 backdrop-blur-sm border-border shadow-xl">
            <CardHeader>
              <CardTitle className="text-foreground flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground font-normal">When you're most active</div>
                </div>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-5">
              <div className="flex items-center justify-between p-4 bg-gradient-to-r from-primary/10 to-purple-500/5 rounded-xl border border-primary/20">
                <span className="text-muted-foreground font-medium">Peak Activity Time</span>
              ...
              <div>
                <span className="text-muted-foreground text-sm font-medium mb-3 block">Top Categories</span>
                <div className="flex flex-wrap gap-2">
                  {metrics?.topCategories?.map((category) => (
                    <Badge key={`cat-${category}`} className="bg-muted hover:bg-muted/80 text-foreground border-border px-3 py-1.5 text-sm font-medium">
                      {category}
                    </Badge>
                  ))}
                </div>
              ...
              <CardHeader>
                <CardTitle className="text-foreground flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground font-normal">Personalized recommendations</div>
                  </div>
                </CardTitle>
              </CardHeader>
                ...
                {insights.map((insight, index) => (
                  <div key={`insight-${insight.title}`} className="flex items-start gap-4 p-5 bg-muted/30 hover:bg-muted/50 rounded-xl border border-border transition-all duration-200">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-muted-foreground text-sm mb-3 leading-relaxed">{insight.message}</p>
