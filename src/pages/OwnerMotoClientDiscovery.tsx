...
export default function OwnerMotoClientDiscovery() {
  ...
  const clientFilters: ClientFilters | undefined = useMemo(() => {
    ...
    if (filters.moto_types && filters.moto_types.length > 0) {
      mapped.motoTypes = filters.moto_types;
    }
    return Object.keys(mapped).length > 0 ? mapped : undefined;
  }, [filters]);

  const { data: clients = [], isLoading, refetch } = useSmartClientMatching(user?.id, 'motorcycle', 0, 10, false, clientFilters);
  ...
  const handleConnect = useCallback(async (clientId: string) => {
    ...
    try {
      ...
      const result = await startConversation.mutateAsync({
        ...
        initialMessage: "Hi! I noticed you are looking for a motorcycle. I have some options that might interest you.",
          ...
          <div className="flex items-center justify-between mb-6 lg:mb-8">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] font-black uppercase tracking-[0.25em] text-orange-500 mb-0.5 animate-pulse">Moto Radar</span>
                <h1 className="text-xl lg:text-2xl font-black tracking-tighter text-foreground">Rider Prospects</h1>
              </div>
            </div>
            <Button 
              variant="outline" 
          ...
          <div className="flex items-center gap-3">
             <div className="relative flex-1">
               ...
               <Input
                 ...
                 className="pl-11 h-12 rounded-2xl bg-muted/20 border-border/20 focus:bg-muted/40 transition-all font-bold"
               />
             </div>
             <button
               onClick={() => refetch()}
          ...
          <aside className="hidden lg:block w-80">
            <div className="sticky top-44 space-y-4">
               <div className="flex items-center justify-between px-2 mb-2">
                 ...
                 {activeFilterCount > 0 && (
                   <button onClick={() => handleApplyFilters({})} className="text-[10px] font-black uppercase tracking-widest text-rose-500">
                     Reset
                   </button>
                 )}
               </div>
            ...
            <AnimatePresence mode="popLayout">
              {isLoading ? (
                ...
                >
                  ...
                  <h3 className="text-xl font-black uppercase tracking-tight mb-2">No Riders Found</h3>
                  <p className="text-muted-foreground max-w-xs mx-auto text-sm font-bold uppercase tracking-widest">
                    Adjust your scan filters to find active riders.
                  </p>
                </motion.div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      ...
      <div className="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
        <Sheet>
          <SheetTrigger asChild>
            <Button size="lg" className="rounded-full h-14 px-8 shadow-2xl bg-orange-600 hover:bg-orange-700 font-black uppercase tracking-widest text-[11px]">
          ...
          <SheetContent side="bottom" className="h-[80vh] rounded-t-[3rem]">
            <div className="px-6 pb-6 overflow-y-auto h-full">
              <SheetHeader className="mb-6">
                <SheetTitle className="font-black uppercase tracking-tight text-2xl">Rider Scope</SheetTitle>
              </SheetHeader>
              <MotoClientFilters
                onApply={handleApplyFilters}
                initialFilters={filters}
