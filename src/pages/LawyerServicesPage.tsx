...
export default function LawyerServicesPage() {
  ...
  return (
    <div className="w-full bg-background relative min-h-screen">
      ...
      <div className="w-full max-w-[1400px] mx-auto px-4 sm:px-8 pt-6 pb-32 relative z-10">
        ...
        <div className="space-y-4 mb-10">
          ...
          <p className={cn('text-base font-bold opacity-40 italic max-w-2xl leading-relaxed', isLight ? 'text-black' : 'text-white')}>
          ...
          <button
            ...
            className={cn('px-5 py-2.5 rounded-full text-[11px] font-black uppercase tracking-widest transition-all border',
              activeCategory === 'all'
                ...
                : isLight ? 'bg-white border-black/10 text-black/60 hover:border-black/20' : 'bg-white/5 border-white/10 text-white/60 hover:border-white/20')}
          >
            All Services
          </button>
          {categories.map((cat) => {
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className={cn('w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-lg transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3', meta.accentBg)}>
                      <CategoryIcon name={meta.icon} className="w-6 h-6" />
                    </div>
                    <Badge variant="outline" className={cn('flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest', meta.ring, meta.accent)}>
                  ...
                  <p className={cn('text-[10px] font-bold uppercase tracking-widest mt-1', meta.accent)}>{meta.label}</p>

                  {pkg.description && (
                    <p className={cn('text-[13px] leading-relaxed opacity-50 mt-3', isLight ? 'text-black' : 'text-white')}>{pkg.description}</p>
                  )}
                  ...
                  <div className={cn('mt-4 pt-4 border-t space-y-2 flex-1', isLight ? 'border-black/5' : 'border-white/5')}>
                    {pkg.features.slice(0, 3).map((f, j) => (
                      <div key={j} className="flex items-start gap-2">
                        <Check className={cn('w-3.5 h-3.5 shrink-0 mt-0.5', meta.accent)} />
                        <span className={cn('text-[11px] font-medium opacity-50', isLight ? 'text-black' : 'text-white')}>{f}</span>
                      </div>
                    ))}
                    {pkg.features.length > 3 && (
                      <span className={cn('text-[10px] font-black uppercase tracking-widest opacity-30 pl-5', isLight ? 'text-black' : 'text-white')}>
