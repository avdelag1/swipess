...
const OwnerInterestedClients = () => {
  ...
  const { data: interestedClients = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const clientIds = likes.map((l: any) => l.user_id);
      ...
      return profiles.map(profile => {
        ...
        const listing = listings?.find(l => l.id === (like as any)?.target_id);
        return {
          ...profile,
          id: (like as any)?.id,
          user_id: profile.user_id,
          target_id: (like as any)?.target_id,
          liked_listing: listing?.title,
          category: 'Applicant',
          created_at: (like as any)?.created_at
        };
      });
    },
    enabled: !!user?.id,
    ...
    <div className="w-full pb-32 bg-background min-h-screen">
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent className="bg-card border-border rounded-[2rem]">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => clientToDelete && removeLikeMutation.mutate(clientToDelete)} className="bg-[#E4007C] text-white rounded-xl font-black">DISMISS</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      {previewClient && (
        <InterestPreviewModal
          ...
          likerId={previewClient.user_id}
          targetId={previewClient.target_id}
          targetType="listing"
