/**
 * Discover Clients Page - Shows swipe cards for owners to find clients
 * Route: /owner/discover-clients
 */
...
function ClientCard({ client, onLike, onMessage }: { client: any; onLike: () => void; onMessage: () => void }) {
  const mainImage = client.images?.[0] || client.profile_images?.[0];
  ...
  const getListingTypeIcon = (type: string | null) => {
    switch (type) {
      case 'moto': case 'motorcycle': return <Car className="w-3 h-3" />;
  ...
  return (
    ...
    >
      <div className="relative h-64 bg-gradient-to-br from-purple-900/30 to-blue-900/30">
        {mainImage ? (
          <img src={mainImage} alt={client.full_name || 'Client'} className="w-full h-full object-cover" />
        ) : (
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          ...
          {client.age && <span className="text-white/70 text-lg">{client.age}</span>}
          {client.verified && <Badge className="bg-blue-500/80 text-white text-xs ml-2">✓ Verified</Badge>}
          {client.city && (
            <div className="flex items-center gap-1 text-white/60 mt-1">
              <MapPin className="w-4 h-4" /><span>{client.city}</span>
            </div>
          )}
        </div>
      </div>
      <div className="p-4 space-y-3">
        ...
        {client.matchPercentage && (
          <div className="flex items-center gap-2">
            ...
            <Badge className={client.matchPercentage >= 80 ? 'bg-green-500/80' : 'bg-yellow-500/80'}>
              {client.matchPercentage}%
            </Badge>
          </div>
        )}
...
export default function OwnerDiscoverClients() {
  ...
  return (
    <div className="h-full flex flex-col bg-black">
      {/* DEBUG INFO */}
      <div className="bg-purple-900/50 p-3 text-xs text-white font-mono border-b border-purple-500/30">
        <div>👤 User: {user?.id ? 'Logged In' : 'NOT LOGGED IN'}</div>
        <div>📊 Clients: {clients?.length || 0} | Filtered: {filteredClients?.length || 0}</div>
        <div>⏳ Loading: {isLoading ? 'YES' : 'NO'} | Error: {error?.message || 'NONE'}</div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-white/50 text-sm">{searchQuery ? 'Try adjusting your search' : 'No client profiles available yet'}</p>
            <Button onClick={handleRefresh} variant="outline" className="mt-4">Refresh</Button>
          </div>
        ) : (
